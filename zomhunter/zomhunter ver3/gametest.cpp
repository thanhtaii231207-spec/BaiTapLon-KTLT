...
void SpawnZombie(Game &g)
{
    ...
    g.zombies.push_back({pos, hp, hp, ZombieSpeed(g.day), ZombieDmg(g.day), g.day/10, 0, true, 0.0f, 0.0f});
}
...
SpriteAssets LoadAssets()
{
    ...
    a.fxKnife = LoadSound("assets/sound_knife.wav");
    a.fxZombieDie = LoadSound("assets/sound_zombie_die.wav");
    return a;
}
...
void UnloadAssets(SpriteAssets &a)
{
    ...
    UnloadSound(a.fxKnife);
    UnloadSound(a.fxZombieDie);
...
void Update(Game &g, float dt, const SpriteAssets &a)
{
    ...
    for(auto &b:g.bullets){
        ...
        for(auto &z:g.zombies){
            ...
            if(sqrtf(dx*dx+dy*dy) < hitRadius){
                ...
                if(z.hp<=0){ z.alive=false; g.score+=10+g.day; PlaySound(a.fxZombieDie); AddParticles(g,z.pos,YELLOW,10,140,4); }
                break;
            }
        }
    }
    ...
    for(auto &bomb:g.bombs){
        ...
        // Khi bom đếm ngược về 0 và chưa nổ
        if(bomb.timer<=0 && !bomb.exploded){
            bomb.exploded=true;
            PlaySound(a.fxExplosion); // Phát âm thanh nổ
            AddParticles(g,bomb.pos,ORANGE,25,200,6); // Tạo hiệu ứng hạt nổ
            
            // QUÉT SÁT THƯƠNG: Trừ máu tất cả zombie trong bán kính nổ
            for(auto &z:g.zombies){
                if(!z.alive) continue;
                float dx=bomb.pos.x-z.pos.x, dy=bomb.pos.y-z.pos.y;
                // Bán kính nổ cố định của bom là 180 pixel
                if(sqrtf(dx*dx+dy*dy) < 180.0f){
                    z.hp -= 600.0f; // Gây 600 sát thương (zombie chết chắc)
                    z.flashTimer=0.3f;
                    if(z.hp<=0){ 
                        z.alive=false; 
                        g.score+=10+g.day; 
                        PlaySound(a.fxZombieDie); // Phát tiếng zombie chết
                        AddParticles(g,z.pos,YELLOW,10,140,4);
                    }
                }
            }
        }
        
        // Nếu bom đã nổ, tăng bán kính vòng tròn hiệu ứng lên để vẽ đồ họa
        if(bomb.exploded) {
            bomb.radius += dt * 600.0f; // Làm hiệu ứng vòng tròn nổ bung ra nhanh hơn
            if(bomb.radius >= 180.0f) bomb.done = true; // Nổ xong thì xóa bom
        }
    }
...
{
    BeginDrawing();
    ClearBackground((Color){10, 10, 8, 255});
    
    if (isOver)
    {
        // 1. Chữ YOU DIED nhỏ ở trên
        DrawText("YOU DIED", SCREEN_W / 2 - 70, SCREEN_H / 2 - 140, 30, RED);

        // 2. Chữ GAME OVER to lớn ở dưới
        DrawText("GAME OVER", SCREEN_W / 2 - 180, SCREEN_H / 2 - 100, 65, RED);

        // 3. Thông số ngày sống sót và điểm số (Đã sửa lỗi WHITE chuẩn)
        DrawText(TextFormat("Survived until Day %d", finalDay), SCREEN_W / 2 - 160, SCREEN_H / 2 - 20, 28, WHITE);
        DrawText(TextFormat("Final Score: %d", score), SCREEN_W / 2 - 100, SCREEN_H / 2 + 20, 28, YELLOW);
    } 
    else 
    {
        DrawText("ZOMBIE SURVIVAL", SCREEN_W / 2 - 220, SCREEN_H / 2 - 140, 56, (Color){220, 30, 30, 255});
