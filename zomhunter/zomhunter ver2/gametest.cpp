...
void Update(Game &g, float dt)
{
    ...
    // ─── PHÍM F: ĐỔI QUA LẠI GIỮA SÚNG VÀ DAO VÔ HẠN ───
    ...
    // ─── LOGIC THÍNH TỰ RƠI KHI HẾT ĐẠN SÚNG THƯỜNG VÀ KHÔNG CÒN SÚNG TRƯỜNG ───
...
void DrawGame(const Game &g, const SpriteAssets &a)
{
    ...
    EndMode2D(); // Đóng camera 2D trước khi vẽ UI/HUD cố định

    // ─── KHU VỰC HUD: LUÔN CỐ ĐỊNH TRÊN MÀN HÌNH ───
    if (currentWeapon == 1) {
    ...
    // ĐÃ CHỈNH SỬA: Chuyển màu chữ sang WHITE (Trắng) và đẩy lên dòng Y=SCREEN_H-25 để thanh hướng dẫn hiển thị siêu rõ!
